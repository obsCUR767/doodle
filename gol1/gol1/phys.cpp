...
void UpdatePhysModel(void* data, float fDeltaTime)
{
    ...
    ph->fAngAccel = (ph->bActionTurnLeft ? ph->YAWTORQUE/ ph->YAWINERTIA : ph->bActionTurnRight ? -ph->YAWTORQUE / ph->YAWINERTIA : 0.0f);
    ...
    rotm2(ph->fAngle, &rot);
    acc.y = 0.0f;
    acc.x = (ph->bActionAccel ? ph->THRUST/ph->MASS : ph->bActionBreak ? -ph->THRUST/ph->MASS : 0.0f);
